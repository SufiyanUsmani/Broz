...
interface Item {
  id: string;  // Changed to string to match CartItem type
  name: string;
  price: number;
}
...
const ProductInfo: React.FC = () => {
  ...
  const addItemToCart = (item: Item) => {
    setAddedToCart(true);
    dispatch(addToCart(item)); // Item id must be string
    setTimeout(() => {
    ...
    <SafeAreaView>
      ...
      <ScrollView>
        <ScrollView horizontal showsHorizontalScrollIndicator={false}>
          {route.params.carouselImages.map((item, index) => (
            <ImageBackground 
              source={{ uri: item }} 
              key={index} 
              style={{ width, height, marginVertical: 5 }} 
              resizeMode="contain"  // Moved resizeMode out of style
            >
        ...
        <View style={{ flexDirection: "row", marginVertical: 5, alignItems: "center", gap: 5 }}>
          ...
          <Text style={{ fontSize: 15, fontWeight: "500" }}>Deliver to suraj - Bangalore 560019</Text>
        </View>
